...
%% ==========================================================================
function [W,B,S] = computeSystemSegregation(descript_roisName,descript_roiOrder)
...
for b=1:length(list_bandNames); bandName = list_bandNames{b};
  ...
  outName = fullfile(outDir,sprintf('SystemSegregation_%s_associationOnly',bandName));
  ...
  outName = fullfile(outDir,sprintf('BetweenSystemMean_%s_associationOnly',bandName));
  cmdStr = sprintf('export_fig %s.png',outName)
  eval(cmdStr);
  h = gcf; savefig(gcf,[outName,'.fig']);
  ...
  outName = fullfile(outDir,sprintf('WithinSystemMean_%s_associationOnly',bandName));
  cmdStr = sprintf('export_fig %s.png',outName)
  eval(cmdStr);
  h = gcf; savefig(gcf,[outName,'.fig']);
  ...
  writetable(toWrite,fullfile(outDir,sprintf('SySTable_%s.csv',bandName)))
...
for b=1:length(list_bandNames); bandName = list_bandNames{b};
  tmpT = readtable(fullfile(outDir,sprintf('SySTable_%s.csv',bandName)));
  tmpD(:,b) = table2array(tmpT(:,2));
end
newTable = tmpT;
newTable.SyS = [];
[newTable] = appendToTable(tmpD,list_bandNames,newTable);
writetable(newTable,fullfile(outDir,'SySTable_allBands.csv'))
