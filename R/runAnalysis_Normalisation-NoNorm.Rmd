...
This report uses SyS column with the 'noNorm' SyS prefix (representing Omit Normalisation).
...
#700
lm_model <- lm(fMRI_craddock_corr_metric_SyS_noNorm ~ age, data = df)
df$fMRI_craddock_corr_metric_SyS_noNorm_ageResid = residuals(lm_model)
```
<br>
...
lm_model <- lm(scale(fMRI_craddock_corr_metric_SyS) ~ scale(ageQuad),
  ...
  tidy() %>%  kable(caption = "Table 1. Effect of Age (NoNorm).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
<br>

```{r}
p <- ggplot(df, aes(x = age, y = fMRI_craddock_corr_metric_SyS_noNorm))
...
ggsave(file.path(outImageDir,'Age_fMRI_NoNorm.png'), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(Cattell_700 ~ scale(fMRI_craddock_corr_metric_SyS_noNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Cattell (NoNorm).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```

```{r}
p <- ggplot(df, aes(y = scale(Cattell_700_ageResid), x = fMRI_craddock_corr_metric_SyS_noNorm_ageResid))
p <- p + geom_point(shape = 21, size = 3, colour = "black", fill = "white", stroke = 2)
p <- p + stat_smooth(method = "lm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
#formatting
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
                     panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Cattell_fMRI_NoNorm.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
lm_model <- lm(Memory_700 ~scale(fMRI_craddock_corr_metric_SyS) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 3. Effect on Memory (NoNorm).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```

<br>
```{r}
p <- ggplot(df, aes(y = scale(Memory_700_ageResid), x = fMRI_craddock_corr_metric_SyS_noNorm_ageResid))
p <- p + geom_point(shape = 21, size = 3, colour = "black", fill = "white", stroke = 2)
p <- p + stat_smooth(method = "lm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
#formatting
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
                     panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Memory_fMRI_NoNorm.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r} 
library(irr)
tmpDF = cbind(df$fMRI_craddock_corr_metric_SyS_chanNorm,df$fMRI_craddock_corr_metric_SyS_noNorm)
icc(tmpDF)
```

```{r}
p <- ggplot(df, aes(x = scale(fMRI_craddock_corr_metric_SyS_chanNorm), y = scale(fMRI_craddock_corr_metric_SyS_noNorm), colour = age))
p <- p + geom_point(shape = 21, size = 3, stroke = 2, alpha = 0.4) + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed')
#formatting
p <- p + xlim(-4.8,4) + ylim(-4.8,4) + #Scaled
#p <- p + xlim(0.5,5) + ylim(0.5,5) + #Non-scaled
  scale_color_gradient(low="green", high="red") +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "right",text = element_text(size=14),
                     panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"ICC_Norm-ClassicVSNoNorm.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
