...
# Notebook: No Global Signal Confound
**Test for effects (Age/Cognition) on SyS that is calculated without a Global Signal Confound regressor during timeseries adjustment.**
<br>
...
Read dataset.
```{r}
df = read.csv(file.path(rawDir,'SyS-NoGlobalSignal.csv')); #head(df)
```
...
df = df[complete.cases(df$fMRI_craddock_corr_metric_SyS_chanNorm), ]
...
#700
lm_model <- lm(fMRI_craddock_corr_metric_SyS_chanNorm ~ age, data = df)
df$fMRI_craddock_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)
```
<br>

## 1) Effect of Age
```{r}
lm_model <- lm(scale(fMRI_craddock_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
                 data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (No Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```

<br>
```{r}
p <- ggplot(df, aes(x = age, y = fMRI_craddock_corr_metric_SyS_chanNorm))
...
ggsave(file.path(outImageDir,'Age_fMRI_noGlobalSignal.png'), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(scale(Cattell_700) ~ scale(fMRI_craddock_corr_metric_SyS) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Cattell (No Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```

```{r}
p <- ggplot(df, aes(y = scale(Cattell_700_ageResid), x = fMRI_craddock_corr_metric_SyS_chanNorm_ageResid))
p <- p + geom_point(shape = 21, size = 3, colour = "black", fill = "white", stroke = 2)
p <- p + stat_smooth(method = "lm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
#formatting
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Cattell_fMRI_noGlobalSignal.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(scale(Memory_700) ~ scale(fMRI_craddock_corr_metric_SyS) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 3. Effect on Memory (No Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```

<br>
```{r}
p <- ggplot(df, aes(y = scale(Memory_700_ageResid), x = fMRI_craddock_corr_metric_SyS_chanNorm_ageResid))
p <- p + geom_point(shape = 21, size = 3, colour = "black", fill = "white", stroke = 2)
p <- p + stat_smooth(method = "lm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
#formatting
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Memory_fMRI_noGlobalSignal.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
