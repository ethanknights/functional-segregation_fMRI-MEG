...
# Notebook: With Global Signal Confound
**Test for effects (Age/Cognition) on SyS that is calculated with a Global Signal Confound regressor during timeseries adjustment.**
<br>
...
lm_model <- lm(scale(fMRI_craddock_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
  ...
  tidy() %>%  kable(caption = "Table 1. Effect of Age (With Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
...
ggsave(file.path(outImageDir,"Age_fMRI_WithGlobalSignal.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(scale(Cattell_700) ~ scale(fMRI_craddock_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Cattell (With Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
...
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Cattell_fMRI_WithGlobalSignal.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(scale(Memory_700) ~ scale(fMRI_craddock_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 3. Effect on Memory (With Global Signal).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
...
p <- p + #xlim() + #ylim(0,400) +
  theme_bw() + theme(panel.border = element_blank(), legend.position = "none",text = element_text(size=14),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size = 1.5),axis.ticks = element_line(colour = "black", size = 1.5))
ggsave(file.path(outImageDir,"Cog-Memory_fMRI_WithGlobalSignal.png"), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
