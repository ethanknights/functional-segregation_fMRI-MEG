...
# Notebook: MEG (Frequency Band - Full Correlation)
**Test for effects (Age/Cognition) on MEG SyS across frequency bands.
<br> 
Full correlation and classic normalisation.**
...
df = df[complete.cases(df$fMRI_schaefer_corr_metric_SyS_chanNorm), ] #still drop fMRI to make ICC simple later
df = df[complete.cases(df$MEG_delta_schaefer_corr_metric_SyS_chanNorm), ]
df = df[complete.cases(df$MEG_theta_schaefer_corr_metric_SyS_chanNorm), ]
df = df[complete.cases(df$MEG_alpha_schaefer_corr_metric_SyS_chanNorm), ]
df = df[complete.cases(df$MEG_beta_schaefer_corr_metric_SyS_chanNorm), ]
df = df[complete.cases(df$MEG_lGamma_schaefer_corr_metric_SyS_chanNorm), ]
df = df[complete.cases(df$MEG_broadband_schaefer_corr_metric_SyS_chanNorm), ]
...
#700
lm_model <- lm(fMRI_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$fMRI_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#delta
lm_model <- lm(MEG_delta_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_delta_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#theta
lm_model <- lm(MEG_theta_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_theta_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#alpha
lm_model <- lm(MEG_alpha_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_alpha_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#beta
lm_model <- lm(MEG_beta_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_beta_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#lGamma
lm_model <- lm(MEG_lGamma_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_lGamma_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)

#broadband
lm_model <- lm(MEG_broadband_schaefer_corr_metric_SyS_chanNorm ~ age, data = df)
df$MEG_broadband_schaefer_corr_metric_SyS_chanNorm_ageResid = residuals(lm_model)
...
```{r}
lm_model <- lm(scale(MEG_delta_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Delta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(scale(MEG_theta_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Theta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(scale(MEG_alpha_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Alpha).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(scale(MEG_beta_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Beta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(scale(MEG_lGamma_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (lGamma).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(scale(MEG_broadband_schaefer_corr_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Broadband).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
<br>

#pivot_longer or melt here to plot all at once as pretty long otherwise!
# ```{r}


tmpDF <- as.data.frame(cbind(df$age,
df$MEG_delta_schaefer_ridgep_metric_SyS_chanNorm,
df$MEG_theta_schaefer_ridgep_metric_SyS_chanNorm))
colnames(tmpDF) = c('age','delta','theta')

tmpDF %>% pivot_longer(agedelta,theta)



# p <- ggplot(df, aes(x = age, y = MEG_delta_schaefer_corr_metric_SyS_chanNorm))
...
```{r}
lm_model <- lm(Memory_700 ~ scale(MEG_delta_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Memory (MEG - Delta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(Memory_700 ~ scale(MEG_theta_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Memory (MEG - Theta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(Memory_700 ~ scale(MEG_alpha_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Memory (MEG - Alpha).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(Memory_700 ~ scale(MEG_beta_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Memory (MEG - Beta).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(Memory_700 ~ scale(MEG_lGamma_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Memory (MEG - lGamma).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
lm_model <- lm(Memory_700 ~ scale(MEG_broadband_schaefer_corr_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
...
<br>
# ```{r}
# p <- ggplot(df, aes(y = scale(Memory_700_ageResid), x = fMRI_schaefer_corr_metric_SyS_chanNorm))
# p <- p + geom_point(shape = 21, size = 3, colour = "black", fill = "white", stroke = 2)
# p <- p + stat_smooth(method = "lm", se = TRUE, fill = "grey60", formula = y ~ x, colour = "springgreen3", size = 3)
# #formatting
# p <- p + #xlim() + #ylim(0,400) +
