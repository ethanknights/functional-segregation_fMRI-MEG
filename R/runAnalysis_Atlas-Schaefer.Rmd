...
# Notebook: fMRI Atlas: Schaefer
**Test for effects (Age/Cognition) on SyS based on Schaefer et al. 2017 ROI parcellation. **
...
df = df[complete.cases(df$fMRI_schaefer_ridgep_metric_SyS_chanNorm), ]
...
## 1) Effect of Age
```{r}
lm_model <- lm(scale(fMRI_schaefer_ridgep_metric_SyS_chanNorm) ~ scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 1. Effect of Age (Schaefer).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
<br>

```{r}
p <- ggplot(df, aes(x = age, y = fMRI_schaefer_ridgep_metric_SyS_chanNorm))
...
ggsave(file.path(outImageDir,'Age_fMRI_Schaefer.png'), width = 25, height = 25, units = 'cm', dpi = 300); p
```
<br>
...
```{r}
lm_model <- lm(Cattell_700 ~ scale(fMRI_schaefer_ridgep_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 2. Effect on Cattell (schaefer).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
...
```{r}
lm_model <- lm(Memory_700 ~scale(fMRI_schaefer_ridgep_metric_SyS_chanNorm) * scale(ageQuad),
               data = df); summary(lm_model) %>% 
  tidy() %>%  kable(caption = "Table 3. Effect on Memory (schaefer).",col.names = c("Predictor", "B", "SE", "t", "p"),  digits = c(0, 2, 3, 2, 3))
```
