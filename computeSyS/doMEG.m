...
%% ------------------------------------------------------------------------
for s = 1:height(t); CCID = t.CCID{s}; 
  
  fN = fullfile(dwDir,['sub-',CCID],...
    'ROIs-Schaefer_100parcels_7networks',...
    sprintf('hilbertEnvCorr_band-%s_roiOrder-dropSchaefer_doOrthog-1.mat',bandName));
...
%     %drop those ROIs from tmpD (& roi info) %DATA ALREADY REMOVED (ORTHOG!)
%     tmpD(:,idx) = [];
